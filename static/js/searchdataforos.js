...
itemTemplate = (item) => {
  let template = "";
  if (page === "servicio_ciudadano.ForosPage") {
    template = `
                      <tr>
                        <td>
                                ...
                                <div class="container mt-4">
                                    <div class="row">
                                        <div class="col-xxl-12 col-xl-12 col-lg-12 col-md-12 col-sm-12">
                                            ...
                                            <p class="fs-6">
                                                Fecha Inicio: ${getdateformat(
                                                  item.start_date
                                                )}<br> 
                                                Fecha Fin: ${getdateformat(
                                                  item.end_date
                                                )} 
                                            </p>
                                        </div>
...
filterbydaterange = (dateRange) => {
  ...
  for (var i = 0; i < dataset.meta.total_count; i++) {
    let itemDateStart = new Date(dataset.items[i].start_date + " 00:00:00");
    let itemDateEnd = new Date(dataset.items[i].end_date + " 00:00:00");
    let found = true;

    if (itemDateStart >= start && itemDateStart <= end && found) {
      filterDataSet.push(dataset.items[i]);
      found = false;
    }
    if (itemDateEnd >= start && itemDateEnd <= end && found) {
      filterDataSet.push(dataset.items[i]);
      found = false;
    }
    if (start >= itemDateStart && start <= itemDateEnd && found) {
      filterDataSet.push(dataset.items[i]);
      found = false;
    }
    if (end >= itemDateStart && end <= itemDateEnd && found) {
      filterDataSet.push(dataset.items[i]);
      found = false;
    }
  }
...
filterbyyearmonth = (yearmonth) => {
  ...
  for (var i = 0; i < dataset.meta.total_count; i++) {
    let itemDate = new Date(dataset.items[i].start_date);
  ...
  for (var i = 0; i < auxfilter.length; i++) {
    let itemDate = new Date(auxfilter[i].start_date);
    let itemmonth = itemDate.getMonth();
    let found = true;
    if (itemmonth === month && found) {
      filterDataSet.push(dataset.items[i]);
    }

    itemDate = new Date(auxfilter[i].end_date);
