...
itemTemplate = (item) => {
  let template = "";
  if (page === "sala_prensa.NoticiaPage") {
    template = `<tr>
      <td>
  ...
  if (page === "misional.UnidadResultadoPage") {
    template = `<tr>
      <td>
          <a href="${item.meta.html_url}">
              <div class="container mt-4">
  ...
  if (page === "sala_prensa.GaleriaPage") {
    template = `<tr>
      <td>
          <a href="${item.meta.html_url}">
              <div class="container mt-4">
                  <div class="row">
                      <div class="col-xxl-6 col-xl-6 col-lg-4 col-md-12 col-sm-12">
                          <img src="${
                            item.image_data.url
                          }" class="img-fluid" alt="${item.image_data.alt}">
                      </div>
                      <div class="col-xxl-6 col-xl-6 col-lg-8  col-md-12 col-sm-12">
                          <h4 class="news-subtitle news-list-item-title">${
                            item.title
                          } </h4>
  ...
  if (page === "sala_prensa.InfografiaPage") {
    template = `<tr>
      <td>
          <a href="${item.meta.html_url}">
              <div class="container mt-4">
                  <div class="row">
                          ...
                          <img src="${item.image_data.url}" class="img-fluid" alt="${item.image_data.alt}">
  ...
  if (page === "sala_prensa.videosPage") {
    template = `
                    <tr>
                        <td>
                            <a href="${item.meta.html_url}">
                                    ...
                                    <div class="row">
                                        <div class="col-sm-12 video-container" >
                                            <div class="video-responsive">
                                                <iframe src="${item.link_video}" frameborder="0" allowfullscreen></iframe>
                                            </div>
                                            <p class="video-list-item-title"> ${item.title} </p>
                                        </div>
                                    </div>
                                </div>
                            </a>
  ...
  if (page === "sala_prensa.audioPage") {
    template = `<tr>
      <td>
          <a href="${item.meta.html_url}">
              <div class="container mt-4">
                  <div class="row">
                      <div class="col-xxl-4 col-xl-4 col-lg-12  col-md-12 col-sm-12 d-flex justify-content-center">
                        <audio controls>
                            <source
                                src="${item.audio.url}"
                                type="audio/mp3">
  ...
  if (page === "sala_prensa.documentoPage") {
    template = `<tr>
      <td>
          <a href="${item.meta.html_url}">
          <div class="container mt-4">
  ...
  if (page === "ministerio.PresentacionPage") {
    template = `<tr>
      <td>
          <a href="${item.meta.html_url}">
          <div class="container mt-4">
  ...
  if (page === "ministerio.ResultadosPage") {
    template = `<tr>
      <td>
          <a href="${item.meta.html_url}">
          <div class="container mt-4">
...
getDataSet = () => {
  var total_count = 0;
  /*fetch(`/api/v2/pages/?type=${page}&fields=*&limit=15`)
    .then((res) => res.json())
    .then(function (data) {
      total_count = data.meta.total_count;
      document.getElementById("filterData").style.display="none";
      fetch(`/api/v2/pages/?type=${page}&fields=*&limit=100`)
        .then((res) => res.json())
        .then(function (data) {
          dataset = data;
    ...
    fetch(`/api/v2/pages/?type=${page}&fields=*&limit=400`)
        .then((res) => res.json())
        .then(function (data) {
          dataset = data;
        })
